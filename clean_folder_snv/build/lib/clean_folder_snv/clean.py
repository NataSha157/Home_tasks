...
def create_folder (path: pathlib.Path) -> pathlib.Path:
...
def parse_recursion(path: pathlib.Path, path_path: pathlib.Path):

    for i in path.iterdir():
        if i.is_dir(): #Папки не перейменовуємо, бо вони видаляються
            parse_recursion(i, path_path)
...
def main():
    ...
    parse_recursion(path, path_path)
    os.rmdir(path)
...
# main(pathlib.Path('D:\Python\Python_less_new\Task_7\TEST'))
...
#     os.rmdir(path)   # існує Path.rmdir(), Path.unlink(), os.remove(), os.unlink()
#
#     print('Перелік усіх розширень, що зустрічались при роботі скрипта', set(end_sort_direct))
#     print("Перелік усіх розширень, які скрипту не відомі", set(end_unknowns))
#     print("Перелік файлів в кінцевій папці 'documents'", res_docs)
